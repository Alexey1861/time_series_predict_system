...
def cnn_model(df, future):
    ...
    model = keras.Sequential([
        keras.layers.Conv1D(filters=64, kernel_size=5, activation='tanh'),
        keras.layers.MaxPooling1D(pool_size=2),
        keras.layers.Conv1D(filters=128, kernel_size=5, activation='tanh'),
        keras.layers.MaxPooling1D(pool_size=2),
    ...
    ax.plot(data[data.index > data.shape[0] - 100]['exp_close'] * scaler.scale_[1] + scaler.mean_[1], color='yellow', label='Экспоненциальная скользящая средняя')
